...
function APIUpdateAttr_hasResponse(props) {
    // props = {url: .., body: , success: , error: , method: ,}
    props = props || {};
    var success_message = props.success_message || 'Update Successfully!';
    var fail_message = props.fail_message || 'Error occurred while updating.';
    $.ajax({
        ...
        toastr.error(fail_message);
        if (typeof props.error === 'function') {
            return props.error(jqXHR, textStatue, errorThrown);
        }
    });
  // return true;
}
...
jumpserver.initDataTable = function (options) {
    ...
    if (options.columnDefs) {
        if (options.columnDefs[0].targets != 0) {
            columnDefs = options.columnDefs.concat(columnDefs)
        }
        else
    ...
    var table = ele.DataTable({
        ...
        dom: options.dom || '<"#uc.pull-right col-md-3" f><"#uc.pull-right col-md-2" l>t<"col-md-4 pull-right" p><"pull-right col-md-4 text-center"i>',
        language: {
