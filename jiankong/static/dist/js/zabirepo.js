...
var int = {
	ready : function() {
		...
		// //登陆
		// for API Login
		// var arr = document.cookie.match(new RegExp("(^| )"+"zbx_sessionid"+"=([^;]*)(;|$)"));
		// 	if(arr != null){
		// 		 var result1=unescape(arr[2]);
		// 		 // alert(unescape(arr[2]))
		// 	   }else{
		// 		 return null;
		// 	   }
		server = new $.jqzabbix(options);
			// alert(server.getApiVersion())

		server.getApiVersion().then(function() {

			// alert(zbxApi.auth.get())
			return zbxApi.auth.get();
		}, function() {
			$.unblockUI(blockUI_opt_all);
			alertDiag(server.isError);
		}).then(function(data) {
			// alert(zbxApi.auth.success(data)
	...
	createGraphTable : function() {
		...
		if (resultObj.length === 0) {
			$('<div class="center-block"><strong>没发现此监控项</strong></div>').appendTo("#table");
			return;
		}
